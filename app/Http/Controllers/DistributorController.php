...
class DistributorController extends Controller
{
    ...
    public function distributorlist_new_arrival(Request $request)
    {
        $result = UsersInfo::leftJoin('tbl_area as stateNew', function($join) {
          ...
          ->whereIn('usersinfo.user_type',['fsc','bsc','dsc'])
          ->where('usersinfo.is_deleted', '=', 'no')
          ->where('usersinfo.new_list_to_view', '=', 'y')
         ->select(   'stateNew.name as state',
         'districtNew.name as district',
         'talukaNew.name as taluka',
         'cityNew.name as city',
    ...
    public function farmermeetingadd_distributorapp(Request $request)
    {
        try {
            ...
            foreach ($meetingFarmerID as $key => $farmer_id_to_get_info) {
                try{ 
                    $farmerdetails=$this->commonController->getFarmerNameById($farmer_id_to_get_info);
                    $farmerMeetingDetails = new FarmerMeetingDetails();
                    $farmerMeetingDetails->farmer_meeting_table_id = $farmerMeetingTableID;
                    $farmerMeetingDetails->farmer_id = $farmer_id_to_get_info;
    ...
    public function farmermeetinglist_distributorapp(Request $request)
{
    try {
        ...
        'tbl_farmer_meeting.photo_five', 'tbl_farmer_meeting_details.farmer_id', 'tbl_farmer_meeting_details.farmer_fname', 'tbl_farmer_meeting_details.farmer_mname', 'tbl_farmer_meeting_details.farmer_lname','tbl_farmer_meeting_details.*',
        'stateNew.name as state_name',
        'districtNew.name as district_name',
         'talukaNew.name as taluka_name',
         'cityNew.name as city_name')
            ->leftJoin('tbl_farmer_meeting_details', 'tbl_farmer_meeting.id', '=', 'tbl_farmer_meeting_details.farmer_meeting_table_id')
            ->leftJoin('tbl_area as stateNew', 'tbl_farmer_meeting.state', '=', 'stateNew.location_id')
            ->leftJoin('tbl_area as districtNew', 'tbl_farmer_meeting.district', '=', 'districtNew.location_id')
    ...
    public function sct_resultadd_images_distributorapp(Request $request)
    {
        ...
        try
        {
            ...
            {   
                // $photo_two_lat_long=explode("_",$request->lat_long_string);
                $photo_one_lat_long=explode("_",$request->lat_long_string);
                $photo_two_lat = $photo_one_lat_long[0]; //2
                $photo_two_long = $photo_one_lat_long[1]; //3
                $filename=$this->processUpload($request, $inputfilenametoupload,$imagedataPath,$photoName);
                $sctresult=SCTResult::where('id',$idLastInserted)->update(['photo_two'=>$filename,'photo_two_lat'=>$photo_two_lat,'photo_two_long'=>$photo_two_long]);
            ...
            {     
                // $photo_three_lat_long=explode("_",$request->lat_long_string);
                $photo_one_lat_long=explode("_",$request->lat_long_string);
                $photo_three_lat = $photo_one_lat_long[0];
                $photo_three_long = $photo_one_lat_long[1];
                $filename=$this->processUpload($request, $inputfilenametoupload,$imagedataPath,$photoName);
                $sctresult=SCTResult::where('id',$idLastInserted)->update(['photo_three'=>$filename,'photo_three_lat'=>$photo_three_lat,'photo_three_long'=>$photo_three_long]);
            ...
            {   
                // $photo_four_lat_long=explode("_",$request->lat_long_string);
                $photo_one_lat_long=explode("_",$request->lat_long_string);
                $photo_four_lat = $photo_one_lat_long[0];
                $photo_four_long = $photo_one_lat_long[1];
                $filename=$this->processUpload($request, $inputfilenametoupload,$imagedataPath,$photoName);
                $sctresult=SCTResult::where('id',$idLastInserted)->update(['photo_four'=>$filename,'photo_four_lat'=>$photo_four_lat,'photo_four_long'=>$photo_four_long]);
            ...
            {     
                // $photo_five_lat_long=explode("_",$request->lat_long_string);
                $photo_one_lat_long=explode("_",$request->lat_long_string);
                $photo_five_lat = $photo_one_lat_long[0];
                $photo_five_long = $photo_one_lat_long[1];
                $filename=$this->processUpload($request, $inputfilenametoupload,$imagedataPath,$photoName);
                $sctresult=SCTResult::where('id',$idLastInserted)->update(['photo_five'=>$filename,'photo_five_lat'=>$photo_five_lat,'photo_five_long'=>$photo_five_long]);
